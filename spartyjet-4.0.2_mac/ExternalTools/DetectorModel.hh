...
namespace DetectorModel
{
  ...
  // A very simple 4-vector class that caches e, eta, phi
  class SimpleJet {
  public:
    SimpleJet(const fastjet::PseudoJet& pj) : _e(pj.e()), _eta(pj.eta()), _phi(pj.phi()), _m(pj.m()), _user_index(pj.user_index()) {}
    SimpleJet(double e=0, double eta=0, double phi=0) : _e(e), _eta(eta), _phi(phi), _m(0), _user_index(-1) {}
    
    int user_index() const {return _user_index;}
    
    double e()              const {return _e;}
    double pseudorapidity() const {return _eta;}
    double phi()            const {return _phi;}
    double m()              const {return _m;}
    
    double p()  const {return std::sqrt(_e*_e - _m*_m);}
    double px() const {return p() * std::cos(_phi) / std::cosh(_eta);}
    double py() const {return p() * std::sin(_phi) / std::cosh(_eta);}
    double pz() const {return p() * std::tanh(_eta);}
    
    double operator[](int i) const {switch(i) {case 0: return px(); break; case 1: return py(); break; case 2: return pz(); break; case 3: return e();} throw "[SimpleJet] Invalid index";}
    void operator*= (double c) {_e *= c; _m *= c;}
  private:
    double _e, _eta, _phi, _m;
    int _user_index;
  }; 


  typedef SimpleJet               particle_t;
  ...
  public:
    ...
    virtual std::vector<fastjet::PseudoJet> smear(const std::vector<fastjet::PseudoJet>& input);
...
{
  return particle_t(e, eta, phi);
   /*particle_t(e/std::cosh(eta)*std::cos(phi),
...
inline std::vector<fastjet::PseudoJet> DetectorModel::RadialSmearing::smear(const std::vector<fastjet::PseudoJet>& inputs)
{
  DetectorModel::container_t particles, smeared;
  for (size_t i = 0; i < inputs.size(); ++i)
    particles.push_back(inputs[i]);
  smeared = smear(particles);
  std::vector<fastjet::PseudoJet> outputs;
  for (size_t i = 0; i < smeared.size(); ++i)
    outputs.push_back(fastjet::PseudoJet(smeared[i]));
  return outputs;
}
